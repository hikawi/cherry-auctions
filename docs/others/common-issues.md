...
This document aims to provide a list of gotchas or common issues that developers
or contributors might run into while working with the project.

## Fast-travel

- Frontend Issues:
  - [Playwright is not supported on your OS](#playwright-unsupported-linux-distributions)
...
> TL;DR. Use `scripts/playwrightd.sh` for spinning up a Playwright server with
> contained browsers. Then use `scripts/playwright.sh` or set the environment
> variable `PW_TEST_CONNECT_WS_ENDPOINT` to the playwright server you spun up.
